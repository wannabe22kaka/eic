...
class EndofperiodinflationPipeline(object):
    def __init__(self):
        self.tablename = year + "EndofperiodInflation"
        self.fileName = year + "EndofperiodofInflation" + ".json"
        ...
        try:
          ...
          self.insert_YearList(year,"EndofperiodInflationListYear")
    ...
    def create_table(self, table):
        try:
            self.sqlstring = "CREATE TABLE IF NOT EXISTS " + table + "( country varchar(30) NOT NULL,  des  text NULL ,units  varchar(20) NULL, percent  float(10,3) NULL, PRIMARY KEY (country)) ENGINE=InnoDB DEFAULT CHARSET=utf8;"
            self.cursor.execute(self.sqlstring)
            result = self.cursor.fetchone()
            exist = False
            if not result:
