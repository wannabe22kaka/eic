...
sys.path.append("/home/jongeun/crawlers/gdpgrowratecrawler/gdpgrowratecrawler")


from items import GdpgrowratecrawlerItem
...
class gdpSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for sel in response.xpath('//*[@id="ctl00"]/div[re:test(@class,"fancy")]/table[contains(tbody, "")]/tr'):
            item = GdpgrowratecrawlerItem()
            item['country'] = self.listToEncoding(sel.xpath('td[1]/text()').extract())
            item['growrate']  = self.listTostring(sel.xpath('td[6]/text()').extract())
